...
public class MyMaze {
	...
	private class Cell {
		...
		Cell(final int x, final int y, final boolean isWall) {
			this.x = x;
			this.y = y;
			this.wall = isWall;
		}
		...
		void addNeighbor(final Cell other) {
	...
	private final int gridDimensionX, gridDimensionY; // dimension of output
														// grid
	...
	public MyMaze(final int aDimension) {
		// Initialize
		this(aDimension, aDimension);
	}

	// constructor
	public MyMaze(final int xDimension, final int yDimension) {
	...
	private void generateMaze(final Cell startAt) {
		...
		while (!cells.isEmpty()) {
			...
			final Cell[] potentialNeighbors = new Cell[] { getCell(cell.x + 1, cell.y), getCell(cell.x, cell.y + 1),
					getCell(cell.x - 1, cell.y), getCell(cell.x, cell.y - 1) };
			for (final Cell other : potentialNeighbors) {
			...
			// get random cell
			final Cell selected = neighbors.get(random.nextInt(neighbors.size()));
			// add as neighbor
	...
	private void generateMaze(final int x, final int y) {
		generateMaze(getCell(x, y)); // generate from Cell
	}

	public char[][] get2DMazeArray() {
		// run it
		final MyMaze maze = new MyMaze(20);
		maze.solve();
		return this.grid;
	}
	...
	public Cell getCell(final int x, final int y) {
		try {
			return cells[x][y];
		} catch (final ArrayIndexOutOfBoundsException e) { // catch out of
															// bounds
			return null;
		}
	}
	...
	public double getProjectedDistance(final Cell current, final double travelled, final Cell end) {
	...
	public void solve(final int startX, final int startY, final int endX, final int endY) {
		// re initialize cells for path finding
		for (final Cell[] cellrow : this.cells) {
		...
		final ArrayList<Cell> openCells = new ArrayList<>();
		// cell being considered
		final Cell endCell = getCell(endX, endY);
		if (endCell == null) {
			return; // quit if end out of bounds
		}
		{ // anonymous block to delete start, because not used later
			final Cell start = getCell(startX, startY);
			if (start == null) {
		...
		final boolean solving = true;
		while (solving) {
			if (openCells.isEmpty()) {
				return; // quit, no path
			}
			// sort openCells according to least projected distance
			Collections.sort(openCells, (cell1, cell2) -> {
				final double diff = cell1.projectedDist - cell2.projectedDist;
				if (diff > 0) {
					return 1;
				} else if (diff < 0) {
					return -1;
			...
			final Cell current = openCells.remove(0); // pop cell least
														// projectedDist
			if (current == endCell) {
				break; // at end
			}
			for (final Cell neighbor : current.neighbors) {
				final double projDist = getProjectedDistance(neighbor, current.travelled + 1, endCell);
	...
	public void updateGrid() {
		final char backChar = ' ', wallChar = '1', cellChar = '0', pathChar = '0';
		// fill background
		for (int x = 0; x < gridDimensionX; x++) {
			for (int y = 0; y < gridDimensionY; y++) {
				grid[x][y] = backChar;
		...
		for (int x = 0; x < dimensionX; x++) {
			for (int y = 0; y < dimensionY; y++) {
				final Cell current = getCell(x, y);
				final int gridX = x * 4 + 2, gridY = y * 2 + 1;
				if (current.inPath) {
